...
uploaded_file = st.file_uploader(
    "Upload your file", type=["xls", "xlsx", "csv"], accept_multiple_files=False
)
...
if uploaded_file is not None:
    ...
    if school_value:
        st.write("### 🎯 Schedules Filtered to Your Requirements")
        final_df = df[
            (df[semester_option] == semester_value) &
            (df[section_option] == section_value) &
        ...
        # Reverse column selection
        st.write("### 📌 Customize Your Final Output")
        select_all = st.button("Select All Columns")
        deselect_all = st.button("Deselect All Columns")

        if select_all:
            selected_columns = final_df.columns.tolist()
        elif deselect_all:
            selected_columns = []
        else:
        ...
        if not selected_columns:
            ...
            # Show condensed DataFrame
            st.write("### 🎯 Condensed Data Preview:")
            concise_df = final_df[selected_columns]
            st.dataframe(concise_df)

            # Download button for CSV export
            csv_data = concise_df.to_csv(index=False).encode("utf-8")
            st.download_button(
            ...
            st.write(
                f"Rows in output may still include other classes with the same Section, "
                f"but it's easier to read **{len(concise_df)} rows** instead of **{len(df)} rows**."
            )
else:
